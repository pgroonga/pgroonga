...
PGDLLEXPORT PG_FUNCTION_INFO_V1(pgroonga_match_positions_character);

void
PGrnInitializeMatchPositionsCharacter(void)
{
	keywordsTable = grn_table_create(ctx,
									 NULL,
									 0,
									 NULL,
...
#define MAX_N_HITS 16
			...
			nHits = grn_pat_scan(ctx,
								 (grn_pat *) keywordsTable,
								 string,
								 stringLength,
								 hits,
								 MAX_N_HITS,
			...
			for (i = 0; i < nHits; i++)
			{
				...
				while (stringForNCharacters < end)
				{
					int characterLength;
					characterLength =
						grn_charlen(ctx, stringForNCharacters, end);
...
#undef MAX_N_HITS
...

	{
		...
		lbs[0] = 1;
		lbs[1] = 1;
		positions = construct_md_array(
			elements, NULL, 2, dims, lbs, INT4OID, sizeof(int32_t), true, 'i');
		pfree(elements);
	}
...
/**
 * pgroonga.match_positions_character(target text,
 *                                    keywords text[]) : integer[2][]
 *
 * pgroonga.match_positions_character(target text,
 *                                    keywords text[],
 *                                    indexName cstring) : integer[2][]
 */
Datum
