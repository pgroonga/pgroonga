...
#ifdef PGRN_SUPPORT_WAL_RESOURCE_MANAGER
static void
PGrnWALBulkInsertStartCustom(PGrnWALData *data, grn_obj *table)
{
...
void
PGrnWALBulkInsertStart(PGrnWALData *data, grn_obj *table)
{
	if (!data)
		return;
	...
	data->table = table;
	data->bulkInserting = true;
#ifdef PGRN_SUPPORT_WAL_RESOURCE_MANAGER
	PGrnWALBulkInsertStartCustom(data, table);
#endif
}

#ifdef PGRN_SUPPORT_WAL_RESOURCE_MANAGER
static void
PGrnWALBulkInsertFinishCustom(PGrnWALData *data)
{
	if (!PGrnWALResourceManagerEnabled)
		return;
...
void
PGrnWALBulkInsertFinish(PGrnWALData *data)
{
	if (!data)
		return;

#ifdef PGRN_SUPPORT_WAL_RESOURCE_MANAGER
	PGrnWALBulkInsertFinishCustom(data);
#endif
	data->bulkInserting = false;
}
...
#ifdef PGRN_SUPPORT_WAL
static void
PGrnWALInsertColumnValueBulkGeneric(PGrnWALData *data,
									const char *name,
									size_t nameSize,
									grn_obj *value)
{
	PGrnWALInsertColumnValueRawGeneric(data,
									   name,
									   nameSize,
									   value->header.domain,
									   GRN_BULK_HEAD(value),
...
#ifdef PGRN_SUPPORT_WAL_RESOURCE_MANAGER
static void
PGrnWALInsertColumnValueBulkCustom(PGrnWALData *data,
								   const char *name,
								   size_t nameSize,
								   grn_obj *value)
{
...

	{
		...
		{
			PGrnWALRecordInsertWriteColumnValueBulk(
				buffer, name, nameSize, value);
		}
	}
}
...
{
#ifdef PGRN_SUPPORT_WAL
	PGrnWALInsertColumnValueBulkGeneric(data, name, nameSize, value);
#endif
#ifdef PGRN_SUPPORT_WAL_RESOURCE_MANAGER
	PGrnWALInsertColumnValueBulkCustom(data, name, nameSize, value);
#endif
}
									 ...
									 grn_obj *value)
{
...

	{
		...
		{
			PGrnWALRecordInsertWriteColumnValueVector(
				buffer, name, nameSize, value);
		}
	}
}
